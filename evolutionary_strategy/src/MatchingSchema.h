...
struct matching_schema
{
		...
		matching_schema(size_t a, size_t b, size_t c, size_t d, const T x,
				bool def) :
				n(a), m(b), p1(c), p2(d)
		{
			ms = new T*[n];
		...
		void set_identity(std::string& sigma1, std::string& sigma2, T value)
		{
			for (size_t i = 0; i < n; ++i)
				for (size_t j = 0; j < m; ++j)
					if (sigma1[i] == sigma2[j]) ms[i][j] = value;
		}

		// e.g., if value == false, ms[i][j] == false means that i and j are in match
		void set_general(const std::string& sigma1, const std::string& sigma2,
				T value)
		{
			for (int i = 0; i < n; ++i)
				for (int j = 0; j < m; ++j)
					if (p1 != 0 && p2 != 0 && (i / p2 == j / p1)) ms[i][j] =
							value;
					else
						ms[i][j] = !value;
		}

		void set_general_fibre(const std::vector<std::string>& sigma1,
				const std::vector<std::string>& sigma2, T value)
		{
			for (int i = 0; i < n; ++i)
				for (int j = 0; j < m; ++j)
					if (p1 != 0 && p2 != 0 && (i / p2 == j / p1)) ms[i][j] =
		...
		void set_constraint(size_t i, size_t j, bool t)
		{
			chi[i][j] = t;
		}

		void print_matching_schema(const std::string& sigma1,
				const std::string& sigma2)
		{
			std::cout << "\t";
			for (int j = 0; j < m; ++j)
				std::cout << sigma2[j] << "\t";
		...
		void shift(int startRow, int startCol, T value)
		{
			...
			for (int i = (startRow); i < n; ++i)
				for (int j = (startCol); j < m; ++j)
					if (p1 != 0 && p2 != 0
							&& ((i - (startRow)) / p2 == (j - (startCol)) / p1)) ms[i][j] =
							value;
