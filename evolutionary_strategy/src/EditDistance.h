...
//(key,value)->(ogni coppia di caratteri delle due sequenze, un set di posizioni di tale coppia(key) nella matrice)
		std::unordered_map<std::pair<int, int>, std::set<std::pair<int, int> >,
				pair_hash>* position;

		void verify_permutation(const std::vector<unsigned short>& perm1,
				const std::vector<unsigned short>& perm2,
				const std::vector<unsigned short>& permOrigin1,
				const std::vector<unsigned short>& permOrigin2,
				const size_t& sig1l, const size_t& sig2l, int p)
		{
			...
			for (int i = 0; i < sig1l; i++)
			{
				...
				{
					std::set<std::pair<int, int> > vO = (*position)[std::pair<
							int, int>(permOrigin1[i], perm2[i])];
					std::set<std::pair<int, int> > vOO = (*position)[std::pair<
							int, int>(perm1[i], perm2[i])];
			...
			if (p != 1)
			{
				for (int i = 0; i < sig2l; i++)
				{
					...
					{
						std::set<std::pair<int, int> > vO =
								(*position)[std::pair<int, int>(permOrigin2[i],
										perm1[i])];
						std::set<std::pair<int, int> > vOO =
								(*position)[std::pair<int, int>(perm2[i],
										perm1[i])];
						for (it = (vO).begin(); it != (vO).end(); ++it)
						{
...
// una distance minore del threshold.
		int incrementalSolution(const std::vector<unsigned short>& a,
				const std::vector<unsigned short>& b, const int& al,
				const int& bl, const std::vector<unsigned short>& sg1,
				const std::vector<unsigned short>& sg2,
				const std::vector<unsigned short>& permOrigin1,
				const std::vector<unsigned short>& permOrigin2,
				const size_t& sg1l, const size_t& sg2l,
				const matching_schema<bool>& m, int p, int threshold)
		{
			...
			verify_permutation(sg1, sg2, permOrigin1, permOrigin2, sg1l, sg2l,
					p);
			...
			if (abs(al - bl) >= threshold || threshold < 0) return -1;
			...
			for (it = (*check).begin(); it != (*check).end(); ++it)
			{
				...
				int max =
						(first > MAX - threshold) ?
								bl : std::min(bl, first + threshold);

				if (min > max) return -1;

				if (min > 1) d[first][min - 1] = MAX;

				if (second < min || second > max || d[first][second] != NOTHING) continue;

				if (firstBoolean)
				{
					firstBoolean = false;
					if (!m.ms[index_of(a[first - 1], sg1, sg1l)][index_of(
							b[second - 1], sg2, sg2l)]) d[first][second] =
							matrix[first - 1][second - 1];
					else
						d[first][second] = 1
								+ std::min(
										std::min(matrix[first][second - 1],
					...
					if (d[first][second] != matrix[first][second])
					{
						...
						if (first < al)
						{
							//down
							(*check).insert(
									std::pair<int, int>(first + 1, second));
							...
							if (second < max)
							{
								...
								//right
								(*check).insert(
										std::pair<int, int>(first, second + 1));
							}
						}
					}
					else
				...
				else
				{
					if (!m.ms[index_of(a[first - 1], sg1, sg1l)][index_of(
							b[second - 1], sg2, sg2l)]) d[first][second] =
							(d[first - 1][second - 1] != PASSED
									&& d[first - 1][second - 1] != NOTHING) ?
									d[first - 1][second - 1] :
									matrix[first - 1][second - 1];
					else
						d[first][second] =
			...
			unsigned my_dist;
			if (d[al][bl] != PASSED && d[al][bl] != NOTHING) my_dist =
					d[al][bl];
			else
			{
			...
			if (my_dist < threshold) return my_dist;
...
//
		...
		int edit_distance_matching_schema_enhanced_with_diagonal(
				...
				unsigned* sig1, unsigned* sig2, const size_t& sig1l,
				...
				int threshold)
		{
			...
			if (abs(s1l - s2l) >= threshold || threshold < 0) return -1;

			// Permutation index
			unsigned* sig1_index = new unsigned[sig1l];
			for (size_t i = 0; i < sig1l; ++i)
			{
				sig1_index[sig1[i]] = i;
			}
			unsigned* sig2_index = new unsigned[sig2l];
			for (size_t i = 0; i < sig2l; ++i)
			{
				sig2_index[sig2[i]] = i;
			}
			...
			for (int j = 1; j <= s2l; j++)
			{
				...
				{
					if (!m.ms[sig1_index[s1[i - 1]]][sig2_index[charS2]]) d[i] =
							p[i - 1];
					else
			...
			if (valuefinal < threshold) return valuefinal;

			return -1;
		}

		unsigned edit_distance_matching_schema_enhanced(
				const std::vector<unsigned>& a, const std::vector<unsigned>& b,
				const size_t& al, const size_t& bl, unsigned*& sg1,
				unsigned*& sg2, const size_t& sg1l, const size_t& sg2l,
				const matching_schema<bool>& m)
		{
			...
			// Permutation index
			unsigned* sig1_index = new unsigned[sg1l];
			for (size_t i = 0; i < sg1l; ++i)
			{
				sig1_index[sg1[i]] = i;
			}
			unsigned* sig2_index = new unsigned[sg2l];
			for (size_t i = 0; i < sg2l; ++i)
			{
				sig2_index[sg2[i]] = i;
			}
				...
				for (size_t j = 1; j < bl + 1; ++j)
				{
					d[i][j] =
							...
							d[i][j - 1] + 1,						// insertion
									d[i - 1][j - 1]
											+ 1
													* m.ms[sig1_index[a[i - 1]]][sig2_index[b[j
															- 1]]]// if in the matching schema there's a false, they match
													);
				}
			}
		...
		unsigned edit_distance_matching_schema(const std::vector<unsigned>& a,
				...
				const size_t& bl, const matching_schema<bool>& m)
		{

			check = new std::set<std::pair<int, int> >();
			position = new std::unordered_map<std::pair<int, int>,
					std::set<std::pair<int, int> >, pair_hash>();
			...
			for (size_t i = 1; i < al + 1; ++i)
			{
				for (size_t j = 1; j < bl + 1; ++j)
				{
					...
					std::set<std::pair<int, int> >* sett =
							&((*position)[std::pair<int, int>(a[i - 1],
									b[j - 1])]);
		...
		Alignment<int> compute_alignment(const unsigned* a, const unsigned* b,
				...
				const matching_schema<bool>& m)
		{
			...
			for (size_t i = 1; i <= al; ++i)
			{
				for (size_t j = 1; j <= bl; ++j)
				{
					...
					if (d[i][j]
							== (d[i - 1][j - 1] + 1 * m.ms[a[i - 1]][b[j - 1]])) path[i][j] =
							'd';
					else if (d[i][j] == d[i - 1][j] + 1) path[i][j] = 'n';
					else
						path[i][j] = 'o';
				}
			}
		...
		int distance_from_alignment(const Alignment<int>& alignment,
				const std::string& sigma1, const std::string& sigma2,
				const matching_schema<bool>& m, bool is_identity)
		{
			...
			for (; alignment.a[index] != _END_ALIGNMENT; index++)
			{
				...
				if (alignment.a[index] != _GAP_FLAG
						&& alignment.b[index] != _GAP_FLAG)
				{
